...
struct btree_node_t * createBTreeNode(void) {
	struct btree_node_t * result	= malloc(sizeof(struct btree_node_t));
	memset(result, 0, sizeof(struct btree_node_t));
	return result;
}

void appendBTreeKey(struct btree_node_t * tree, uint32_t key, void * location) {
	struct btree_node_t * node = tree;
	uint32_t shr = key;
	for(int f=0; f < 32; f++) {
		if(shr == 0)
			break;
			...
			if(node->right == NULL)
				node->right = createBTreeNode();
			node = node->right;
		} else {
			if(node->left == NULL)
				node->left = createBTreeNode();
			node = node->left;
		}
		shr >>= 1;
	}
